...
template <class T>//e un sablon si nu mai trb sa scriem alea de sus
T suma(T a, T b) {
	return a + b;
}
...
class Vector {
	int lungime;
	Model** elemente;// vectior de pointeri
public:
	Vector() {
		this->lungime = 0;
		this->elemente = NULL;
	...
	Vector(int lungime, Model** elemente) {
		...
		for (int i = 0; i < lungime; i++) {
			this->elemente[i] = new Model(*elemente[i]); //deep copy
		}
	}
	...
	~Vector() {
		for (int i = 0; i < lungime; i++) {
			delete this->elemente[i];//nu punem delete[] ca sa stergem cate un el nu un bloc
		}
		delete[]this->elemente;
	}

	Vector operator =(const Vector& v) {//ii dam vector si nu void ca sa l apelam in cascada
		if (&v != this) {
			this->lungime = v.lungime;
	...
	//met care sa adauge un elem
	void AdaugaElement(Model* element) {
		Model** aux = new Model * [this->lungime + 1];//vect de pointeri 
		for (int i = 0; i < this->lungime; i++) {
			aux[i] = this->elemente[i];//shallow copy
		}
		aux[lungime] = element;
		delete[]this->elemente;//stergem doar vect pt ca am fct shallow copy si trb sa stergem decat adresele
		this->elemente = aux;
		this->lungime++;
	}
	int getLungime() {
...
int main() {
	//declaram vect
	Vector<int>intregi;//ia cls si unde e model inloc cu int
	Vector<Magazin>magazine;
	Magazin* m = new Magazin("Bucuresti", 3, 100, 2024);//aloc spatiu pe heap
	//cu new alocam pe heap si pe steak fara new
