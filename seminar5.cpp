...
public:
	...
	Magazin operator=(const Magazin& m)//se pune const ca sa nu scriem in loc de this alte chestii
	{
		if (&m != this) { //verificare de autoasignare adc sa nu putem sa scriem in main m4=m4
			this->oras = m.oras;
			this->nrAngajati = m.nrAngajati;
			if (this->salarii) {
				delete[]this->salarii;
